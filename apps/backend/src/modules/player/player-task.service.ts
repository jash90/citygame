    ...
    const attempt = await withSerializableRetry(this.prisma, async (tx) => {
      ...
      if (attemptStatus === AttemptStatus.CORRECT || attemptStatus === AttemptStatus.PARTIAL) {
        ...
        if (attemptStatus === AttemptStatus.CORRECT) {
          ...
          if (game?.flowType === 'OPEN_WORLD') {
            nextTaskId = null;
            isComplete = false;
          } else if (game?.flowType === 'BRANCHING' || game?.flowType === 'MIXED') {
            const outgoingTransitions = await tx.taskTransition.findMany({
              where: { gameId, fromTaskId: taskId },
              orderBy: { orderIndex: 'asc' },
              take: 1,
            });
            if (outgoingTransitions.length > 0) {
              nextTaskId = outgoingTransitions[0].toTaskId;
            } else {
              const fallbackNext = await tx.task.findFirst({
                where: { gameId, orderIndex: { gt: task.orderIndex } },
                orderBy: { orderIndex: 'asc' },
              });
              nextTaskId = fallbackNext?.id ?? null;
            }
            isComplete = nextTaskId === null;
          } else {
            // LINEAR (default)
          ...
          if (updatedSession.teamId) {
            await tx.gameSession.updateMany({
              ...
              data: {
                ...
                status: isComplete ? SessionStatus.COMPLETED : SessionStatus.ACTIVE,
